function filename = Generate_ib_vg(lib,corner,modelname,W,L,m,nf,temperature,VGmax,VDD,VG_Step,VDD_sub_Start,VDD_sub_End,VDD_sub_Step,flag_fet,Terminals)
% BIAS
...
%file name
filename=['netlist_ib_vg_' corner '.sp'];
...
fprintf(fileID, '.DC GG %s %s %s sweep VDD %s %s %s \n\n', VG_Start, VG_Stop, VG_Step, VDD_sub_Start, VDD_sub_End, VDD_sub_Step);
...
fprintf(fileID, '.print par(''-1*i(vb)'')\n\n');
