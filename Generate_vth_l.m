function filename = Generate_vth_l(lib,corner,modelname,W,L,m,temperature,VGmax,VDD,VG_Step,VDlin,vdd_sat,wmin,wmax,lmin,lmax,flag_fet,Terminals,shrink,lminmultiplier)
...
multipler = lminmultiplier;
%Lmin = str2double(lmin);
newlmin=lmin*multipler;

for i=1:length(newlmin)
    fprintf(fileID, '+ %s %g 1 \n', wmax, newlmin(i));
end
...
newlmax=lmin*multipler;

for i=1:length(newlmax)
    fprintf(fileID, '+ %s %9.2e 1 \n', wmin, newlmax(i));
end
