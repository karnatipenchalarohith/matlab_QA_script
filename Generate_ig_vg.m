function filename = Generate_ig_vg(lib,corner,modelname,W,L,m,nf,temperature,VGmax,VDD,VG_Step,VDD_ig_Start,VDD_ig_Stop,VDD_ig_Step,flag_fet,Terminals)
% BIAS
...
filename=['netlist_ig_vg_' corner '.sp'];
...
fprintf(fileID, '.DC GG %s %s %s sweep VDD %s %s %s \n\n', VG_Start, VG_Stop, VG_Step, VDD_ig_Start, VDD_ig_Stop, VDD_ig_Step );
...
fprintf(fileID, '.print par(''-1*i(vg)'')\n\n');
